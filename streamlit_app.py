...
# Secrets veya UI Input (Grok API Key)
GROK_API_KEY = st.secrets.get("GROK_API_KEY", st.text_input("Grok API Key (x.ai/api'den alın): ", type="password"))
...
st.markdown("Grok Code Fast API entegrasyonuyla (düşük maliyetli) X/Web taraması. Tablo ve harita gösterir.")
...
if st.button("Analiz Et (Son 30 Gün)"):
    with st.spinner("Grok ile tarama yapılıyor... (X ve web kaynakları çaprazlandı)"):
        ...
        if not events.empty:
            events['komşu_tesisler'] = events.apply(lambda row: row.get('komşu_tesisler', find_neighbors(row.get('lat'), row.get('lng'))), axis=1)
...
st.caption("Kaynaklar: Grok Code Fast API (X postları + web haberleri). Günlük cache ile optimizasyon.")
